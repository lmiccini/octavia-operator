...
import (
	...
	webhookv1beta1 "github.com/openstack-k8s-operators/octavia-operator/internal/webhook/v1beta1"
...
func main() {
	...
	if err := (&controller.OctaviaAPIReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	...
	if err = (&controller.OctaviaRsyslogReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.OctaviaAmphoraControllerReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupOctaviaWebhookWithManager(mgr); err != nil {
