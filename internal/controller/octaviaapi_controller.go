...
import (
	...
	"github.com/openstack-k8s-operators/octavia-operator/internal/octaviaapi"
...
func (r *OctaviaAPIReconciler) GetLogger(ctx context.Context) logr.Logger {
	return log.FromContext(ctx).WithName("Controllers").WithName("OctaviaAPI")
}
...
func (r *OctaviaAPIReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	// index topologyField
	if err := mgr.GetFieldIndexer().IndexField(ctx, &octaviav1.OctaviaAPI{}, topologyField, func(rawObj client.Object) []string {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &octaviav1.OctaviaAPI{}, authAppCredSecretField, func(rawObj client.Object) []string {
...
func (r *OctaviaAPIReconciler) reconcileInit(
	...
	for endpointType, data := range octaviaEndpoints {
		endpointTypeStr := string(endpointType)
		endpointName := octavia.ServiceName + "-" + endpointTypeStr
...
func (r *OctaviaAPIReconciler) reconcileNormal(ctx context.Context, instance *octaviav1.OctaviaAPI, helper *helper.Helper) (ctrl.Result, error) {
	...
	deplDef, err := octaviaapi.Deployment(instance, inputHash, serviceLabels, serviceAnnotations, topology)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
...
func (r *OctaviaAPIReconciler) generateServiceSecrets(
	...
	if instance.Spec.Auth.ApplicationCredentialSecret != "" {
		...
		if err := h.GetClient().Get(ctx, key, secret); err != nil {
			if k8s_errors.IsNotFound(err) {
				Log.Info("ApplicationCredential secret not found, waiting", "secret", key)
		...
		if !okID || len(acID) == 0 || !okSecret || len(acSecret) == 0 {
			Log.Info("ApplicationCredential secret missing required keys", "secret", key)
